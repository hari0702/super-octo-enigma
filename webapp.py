...
def add_task():
    ...
    modules.function.append_todo_file(path, todo)  # Append the new task to the file

st.title("To-Do List App")
st.subheader("Manage your tasks efficiently")
read_tasks = modules.function.read_todo_file(path)

for index, task in enumerate(read_tasks):
   checkbox = st.checkbox(task, key=task)
   if checkbox:
         read_tasks.pop(index)
         modules.function.write_todo_file(path, read_tasks)  # Update the file after removing
